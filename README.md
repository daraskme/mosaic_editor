...
静止画の SAM2 輪郭化では、画像特徴を1枚につき1回だけ計算し、複数の検出領域で共有します。
領域ごとに最良のマスクだけを元解像度へ拡大するため、大きい画像の一時メモリ使用も抑えます。
モデルの読込・初回ダウンロード状況は検出中の画面に表示されます。
SAM2 を読み込めない場合は矩形マスクで続行するため、結果一覧の `[輪郭]` / `[矩形]` を確認してください。
