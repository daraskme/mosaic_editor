...
- **自動検出 (画像)** : 既存のブロックモザイクを画像処理で検出。局部は YOLOv8 → SAM2 で輪郭マスク化
- **自動検出 (動画)** : 既存モザイクは各フレームで検出。局部は SAM2 Video で**全フレームにわたり追跡**
...
| モザイク | ❌ | 既存のブロックモザイク。ドット絵等の誤検出を避けるためデフォルトOFF |

### 動画の自動モザイク

動画を開いて「自動検出」→「動画全体」を選ぶと、既存モザイクは各フレームで検出します。
局部は150フレームごとのキーフレームで検出し、SAM2.1 Video が各対象を
チャンク内全フレームに伝播・追跡します。
...
既存モザイク検出は、均一色の四角いブロックが格子状に並ぶ特徴を利用します。
小さいブロックや強く圧縮された画像では拾いにくく、ドット絵やタイル模様を
拾う場合があるため、検出結果を確認してから適用してください。
...
局部の自動検出用依存 (torch 等) は exe には含まれません (`spec` の `excludes`)。
局部カテゴリを初めて選んだときに実行環境 (~1.5GB) のダウンロードを提案し、
承認すると `uv` 経由で `%LOCALAPPDATA%\MosaicEditor\pydeps` に自動インストールされます
(Visual C++ 再頒布パッケージも自動で導入されます)。
手動編集と `モザイク` 検出はダウンロードなしでそのまま使えます。

フル版として配布したい場合は `excludes` を外してフル依存を入れた環境で
ビルドしてください (数GBになります)。
